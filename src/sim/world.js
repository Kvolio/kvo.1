...
export class World {
  ...
  blankStats() {
    return {
      penetration: 0, maxDepth: 0, channelLength: 0,
  ...
  updateStats() {
    ...
    const backPlane = lastBack.frontX + lastBack.thickness / Math.max(0.15, Math.cos(lastBack.slope * Math.PI / 180));
    ...
    for (let i = 0; i < d.n; i++) {
      ...
      if (d.role[i] === ROLE.ARMOUR) {
        ...
        // enormous bulge.
        if (d.damage[i] < 0.45) {
          const dep = (d.px[i] - fx) * n0[0] + d.py[i] * n0[1];
          const ref = (d.rx[i] - fx) * n0[0] + d.ry[i] * n0[1];
          if (ref > (backPlane - fx) * 0.82) {
            const move = dep - ref;
    ...
    const throughDepth = (backPlane - fx);
    if (!st.perforated && st.maxDepth > throughDepth * 0.995 && penMass > this.penetratorMass0() * 0.02) {
      st.perforated = true;
